...
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.CMD;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.DOCKERFILE;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.JAVA;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.SH;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.XML;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.YAML;
import static de.containerautomat.factory.builders.DfaApplicationBuilder.TargetFileType.YML;
...
class TargetFileCommentBuilder {
    ...
    record TargetFileCommentAttributes(DfaApplicationBuilder.TargetFileType fileType, String commentStart, String commentLinePrefix, String commentEnd, int topLinesToSkip) {
    }


    static final String GENERATOR_COMMENT_TEMPLATE = """
            %3$s
    ...
    static final Map<DfaApplicationBuilder.TargetFileType, TargetFileCommentAttributes> targetFileTypeComments = new EnumMap<>(DfaApplicationBuilder.TargetFileType.class);

    static {
        for (DfaApplicationBuilder.TargetFileType fileType : DfaApplicationBuilder.TargetFileType.values()) {
            switch (fileType) {
                case CMD:
                    targetFileTypeComments.put(CMD, new TargetFileCommentAttributes(fileType, "", "rem", "", 1));
                    break;
                case DOCKERFILE:
                    targetFileTypeComments.put(DOCKERFILE, new TargetFileCommentAttributes(fileType, "", "#", "", 0));
                    break;
                case JAVA:
                    targetFileTypeComments.put(JAVA, new TargetFileCommentAttributes(fileType, "/*", " *", " */", 0));
                    break;
                case SH:
                    targetFileTypeComments.put(SH, new TargetFileCommentAttributes(fileType, "", "#", "", 1));
                    break;
                case YAML:
                    targetFileTypeComments.put(YAML, new TargetFileCommentAttributes(fileType, "", "#", "", 0));
                    break;
                case YML:
                    targetFileTypeComments.put(YML, new TargetFileCommentAttributes(fileType, "", "#", "", 0));
                    break;
                case XML:
                    targetFileTypeComments.put(XML, new TargetFileCommentAttributes(fileType, "<!--", "   ", "-->", 3));
                    break;
                default:
                    break;
            }
    ...
    static String insertGeneratorComment(String sourceText, DfaApplicationBuilder.TargetFileType targetFileType, String generationId) {
        ...
        var targetFileCommentAttributes = targetFileTypeComments.get(targetFileType);
        var comment = TargetFileCommentBuilder.GENERATOR_COMMENT_TEMPLATE.formatted(
                ContainerAutomatFactoryApp.class.getCanonicalName(),
                Instant.now(),
                targetFileCommentAttributes.commentStart(),
                targetFileCommentAttributes.commentLinePrefix(),
                targetFileCommentAttributes.commentEnd(),
                generationId).trim();

        if (targetFileCommentAttributes.topLinesToSkip() == 0) {
        ...
        for (var charIndex = 0; charIndex < sourceText.length(); charIndex++) {
            ...
            if (lineIndex == targetFileCommentAttributes.topLinesToSkip()) {
