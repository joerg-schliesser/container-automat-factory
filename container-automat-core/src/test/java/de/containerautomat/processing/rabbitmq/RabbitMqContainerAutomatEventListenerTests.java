...
/**
 * A test suite for testing the method in {@link RabbitMqContainerAutomatEventListener}
 * that handles events.
 * <p/>
 ...
 */
@SpringBootTest(classes = RabbitMqContainerAutomatEventListener.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
@ActiveProfiles("rabbitmq")
class RabbitMqContainerAutomatEventListenerTests {
    ...
    @Test
    void artemis_containerautomat_event_listener_bean_is_available() {

        assertNotNull(containerAutomatEventListener);
        assertInstanceOf(RabbitMqContainerAutomatEventListener.class, containerAutomatEventListener);
    }
    ...
    void handle_event_logs_event(CapturedOutput output) {
        ...
        var testEvent = ContainerAutomatRuntimeEvent.builder()
                ...
                .build();
        var testMessage = RabbitMqContainerAutomatEventListener.HANDLE_EVENT_LOG_MESSAGE_TEMPLATE.formatted(testEvent.toString());
